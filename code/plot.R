...
# load('~/Documents/Mayo_Research/SemiSimulation/0925/sim/new/sim/allD_res.Rdata')
# res0 <- melt(res)
...
# a = res0[res0$measures =='FDR' & res0$methods=='RAIDA',]
# levels(res0$depth.mus) = c('low','medium','high')
# levels(res0$nOTUs) = c('low','medium','high')
# levels(res0$nSams) = c('low','medium','high')
# levels(res0$covariate.eff.means) = c('low','medium','high')
# 
# res.na <- res0[is.na(res0$value),]
# a = res.na %>% filter(covariate.types =='binary' & methods =='RAIDA' & measures=='FDR')
# ## covariate effect
# res1 <- res0 %>% filter(depth.mus =='medium' & nSams == 'medium' & nOTUs == 'medium')
# 
...
# 
# 
# cov <- data_summary(res1, formula = 'value ~ depth.mus + diff.otu.modes + nOTUs + covariate.types + nSams + covariate.eff.means +diff.otu.pcts +measures+ methods')
# head(cov)
...
# 
# ggplot(cov %>% filter(measures =='TPR' & covariate.types=='binary' & !(methods %in% sub)), aes(x = covariate.eff.means, y = value,  fill = methods)) +
#   theme_bw() +
...
#   labs(y = 'Power', x = stri_trans_totitle(name), color = "Methods") +
#   thw
# 
# ggplot(cov %>% filter(measures =='FDR' & covariate.types=='binary' & !(methods %in% sub)), aes(x = covariate.eff.means, y = value,  fill = methods)) +
#   theme_bw() +
#   geom_bar(position = position_dodge2(width = 0.7, preserve = "single"), stat="identity", width = 0.7) +
#   geom_errorbar(aes(ymax=ymax, ymin=ymin, linetype = NULL),  width=0.7, size = 0.2,position=position_dodge2(.7, preserve = "single")) +
#   scale_fill_manual(values = cols) +
#   # scale_y_continuous(limits = c(0,1),expand = c(0.1, 0, 0, 0)) +
#   facet_grid(as.formula(grid.formula), scales = 'free_y')+
#   labs(y = 'FDR', x = stri_trans_totitle(name), color = "Methods") +
#   thw
# load('/Users/m216453/Documents/Mayo_Research/SemiSimulation/0925/sim/new/sim/D3abundantloglinearSub_L1nOTU_L5continuousnonebalancednonenSam_L2L1low.Rdata')
...
# setwd("/Users/m216453/Documents/Mayo_Research/SemiSimulation/0925/sim/new/sim")
for(name in c('allD0')){
  if(name =='allC0'){
    name1 ='Stool'
  }else{
  ...
  for(covariate.type in c('binary','continuous')){
    ...
    # res1 = res1 %>% filter(methods !=c('LinDA','GMPR+Spearman','GMPR+t-test','Wrench+t-test','GMPR+Wilcoxon','Wrench+Wilcoxon'))
    sub0 = c("Aldex2(Wilcoxon)","ANCOM-BC","Beta-binomial","DACOMP","eBay(t-test)",
             "GLM(quasipoisson)","GMPR+DESeq2","GMPR+edgeR","LDM","mbzinb",
             'DESeq2','Wrench+DESeq2','edgeR', 'Wrench+edgeR','Wrench+metagenomeSeq','eBay(Wilcoxon)','Aldex2(t-test)',
             "RAIDA","Rarefy+t-test","Rarefy+Wilcoxon","TSS+t-test","TSS+Wilcoxon","metagenomeSeq") # for binary
    sub0_sup = c('RioNorm2') # for supplementary binary
    sub1 = c('DESeq2','GMPR+DESeq2','edgeR', 'GMPR+edgeR','eBay(t-test)','ANCOM-BC','DACOMP','LDM','GLM(quasipoisson)','Beta-binomial','Rarefy+Spearman','TSS+Spearman') # for continuous
    ...
    x1 = aggregate(value~., x1, function(x) median(x))
    ...
    kbl(x4, align = 'c') %>%
      ...
      save_kable(file = paste0('/Users/m216453/Documents/Mayo_Research/SemiSimulation/DAMethodsEvaluation/result/SimulationEvaluation/',covariate.type,'_',name1,'_FPs.pdf'))
    
    
    ## For kable of False discover FPs
    res1[res1$value>0,'ct'] = 1
    res1[res1$value==0,'ct'] = 0
    res1 = res1 %>% dplyr::select(-value) 
    
    formula = paste0('ct~ depth.conf.factors + nOTUs + nSams + methods + covariate.types')
    grid.formula = '. ~ nSams'
    
    m <- aggregate(as.formula(formula), res1, function(x) sum(x[!is.na(x)])) %>% mutate(ct = ct/1000)
    head(m)
    ...
    size = 6
    df = m %>% filter(covariate.types==covariate.type& (methods %in% sub))
    ...
    x1 = kb %>% dplyr::select(depth.conf.factors,nOTUs, nSams, methods, shape) #%>% filter(nOTUs =='OTU=50') %>% dplyr::select(-nOTUs)
    head(x1)
    x1$nOTUs = factor(x1$nOTUs, levels = c('OTU=50','OTU=200','OTU=500'))
    x1$nSams = factor(x1$nSams, levels = c('sample=50','sample=100','sample=200'))
    x1$depth.conf.factors = factor(x1$depth.conf.factors, levels = c('None','Depth confounding'))
    ...
    kbl(x4, align = 'c') %>%
      ...
      column_spec(22, color = count.score(data = x4, j = 22,color1 = 'white', color2 = 'black', value = 54),
                  background = count.score(data = x4, j = 22,color1 = brewer.pal(8,'Dark2')[3], color2 = 'white', value = 54),bold = T) %>%
      save_kable(file = paste0(getwd(),'/result/SimulationEvaluation/',covariate.type,'_',name1,'_FDI.pdf'))
    # as_image(width = 7, height = 3, file = paste0(getwd(),'/plot/',covariate.type,'_',name1,'.pdf')) 
    
    if(covariate.type =='binary'){
      df = m %>% filter(covariate.types==covariate.type& (methods %in% c(sub0,sub0_sup)))
    }else{
      df = m %>% filter(covariate.types==covariate.type& (methods %in% c(sub1)))
    }
    df[df$ct > 0.1,'shape'] = 'uncontrol'
    df[df$ct <= 0.05,'shape'] = 'fdr_0.05'
    df[df$ct <= 0.1 & df$ct>0.05,'shape'] = 'fdr_0.1'
    df[df$ct <= 0.2 & df$ct>0.1,'shape'] = 'fdr_0.2'
    ...
    p0 = ggplot(df,aes(x = nOTUs, y = ct, fill = legend)) + # fill = legend shhould be here, or the geom_text label will have same position of label
      ...
      theme_bw() +thw +
      labs(y = 'False discovery iterations', x = 'OTU number', fill = "", shape = "", color = "",
           title = paste0("Default: no differential taxa (", covariate.type,"-",name1,')'), 
           caption = "False discovery iterations = iterations identify false positive / 1000 iterations") +
    ...
    ggsave(paste0('~/Documents/Mayo_Research/SemiSimulation/DAMethodsEvaluation/result/SimulationEvaluation/',name,'_',covariate.type,'.pdf'), width = 25, height =20)
    
    # if(covariate.type =='binary'){
    #   df0 = m %>% filter(covariate.types==covariate.type& (methods %in% sub0_sup))
    #   df0[df0$ct > 0.1,'shape'] = 'uncontrol'
    #   df0[df0$ct <= 0.05,'shape'] = 'fdr_0.05'
    #   df0[df0$ct <= 0.1 & df0$ct>0.05,'shape'] = 'fdr_0.1'
    #   df0[df0$ct <= 0.2 & df0$ct>0.1,'shape'] = 'fdr_0.2'
    #   
    #   p1 = ggplot(df0,aes(x = nOTUs, y = ct, fill = legend)) + # fill = legend shhould be here, or the geom_text label will have same position of label
    #     geom_bar(position = position_dodge(width = 0.8), stat="identity", width = 0.1) +
    #     geom_point(aes(color = legend, shape = shape),  size = 5, position = position_dodge(width = 0.8)) +
    #     geom_text(aes(label=label), position=position_dodge(width=0.8),size=5, vjust = -0.7) + 
    #     facet_grid(nSams~depth.conf.factors, scales = 'free_y') +
    ...
    #     geom_hline(yintercept = 0.2, colour = '#e03531', linetype = 'dashed', size =  0.5) + # red
    #     guides(shape = FALSE, fill = FALSE)
    #   ggsave(paste0('~/Documents/Mayo_Research/SemiSimulation/DAMethodsEvaluation/result/SimulationEvaluation/',name,'_',covariate.type,'_Supplementary.pdf'), width = 20, height =20)
    #   
    # }
...
for(name in c('allC0')){
  ...
  for(covariate.type in c('binary','continuous')){
    ...
    sub0 = c("Aldex2(Wilcoxon)","ANCOM-BC","Beta-binomial","DACOMP","eBay(t-test)",
             ...
             "RAIDA","Rarefy+t-test","Rarefy+Wilcoxon","TSS+t-test","TSS+Wilcoxon","metagenomeSeq") # for binary
    sub0_sup = c('RioNorm2') # for supplementary binary
    sub1 = c('DESeq2','GMPR+DESeq2','edgeR','GMPR+edgeR','ANCOM-BC','DACOMP','LDM','GLM(quasipoisson)','Beta-binomial','Rarefy+Spearman','TSS+Spearman') # for continuous
...
# ggplot(df, aes(x = nOTUs, y = ct,  fill = legend)) +
#   theme_bw() +
#   geom_bar(position = position_dodge2(width = 0.7, preserve = "single"), stat="identity", width = 0.7) +
...
#   facet_grid(nSams~depth.conf.factors, scales = 'free_y')+
#   labs(y = 'FDR', x = 'OTU number', fill = "") +
#   thw +
#   geom_hline(yintercept = 0.05, colour = 'red', linetype = 'dashed', size =  0.5)
# ggsave('/Users/m216453/Documents/Mayo_Research/SemiSimulation/0925/sim/new/plot/allC0_binary.pdf', width = 20, height = 16)

# p1 = ggplot(m %>% filter(covariate.types=='continuous' & !(methods %in% sub)), aes(x = nOTUs, y = ct,  fill = methods)) +
#   theme_bw() +
#   geom_bar(position = position_dodge2(width = 0.7, preserve = "single"), stat="identity", width = 0.7) +
#   scale_fill_manual(values = cols) +
#   scale_y_continuous(expand = c(0.1, 0, 0, 0)) +
#   facet_grid(depth.conf.factors~nSams, scales = 'free_y')+
#   labs(y = 'FDR', x = 'OTU number', fill = "") +
#   thw +
#   geom_hline(yintercept = 0.05, colour = 'red', linetype = 'dashed', size =  0.5)
# ggsave('/Users/m216453/Documents/Mayo_Research/SemiSimulation/0925/sim/new/plot/allC0_continuous.pdf', width = 20, height = 16)
