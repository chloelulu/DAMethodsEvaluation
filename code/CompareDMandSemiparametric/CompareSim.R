...
## Below code are comparison of Semi-Simulation Framework, Dirichlet multinomial Simulation and Original data
...
source('code/CompareDMandSemiparametric/Func.R')
...
load('data/CompareDMandSemiparametric/Stool_V35.RData') # OTU dataset
load('data/CompareDMandSemiparametric/Stool_V35_dirmult.Rdata') # For saving computational time, already prepared dirmult.paras
otu.tab = Stool_V35
name = 'Stool'
...
# dim(otu.tab)
load(paste0('data/CompareDMandSemiparametric/','Semi_',name,'.Rdata'))
load(paste0('data/CompareDMandSemiparametric/','DM_',name,'.Rdata'))
...
sink(paste0('result/CompareDMandSemiparametric/',name,'_Sparsity.txt'))
...
sink()
cols = c(`Dirichlet multinomial` = '#3db7e4',Semiparametric = '#ff8849', `Raw dataset`= '#69be28')

cbind(`Dirichlet multinomial` = zero3, `Raw dataset` = zero2, `Semiparametric` = zero4) %>% melt() %>% 
  ggplot(aes(x = X2, y = value, fill = X2)) + 
  geom_violin() +
  ...
  ggtitle(name)
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_Sparsity_violin.pdf'), plot = last_plot(), width = 8, height =3.5, dpi = 60, units = 'in')
...
sink(paste0('result/CompareDMandSemiparametric/',name,'_Prevelance.txt'))
cat('Raw median, max, min \n')
...
qq1 = cbind(`Dirichlet multinomial` = zero3, `Raw dataset` = zero2, `Semiparametric` = zero4) %>% as.data.frame() %>% rownames_to_column('OTU')

head(qq1)

colnames(qq)[2:4] = c('Dirichlet multinomial','Raw dataset','Semiparametric')
ggplot(melt(qq),aes(x = variable, y = value, fill = variable)) + 
  geom_violin() +
  ...
  theme(axis.text.x = element_blank(),
        ...
        plot.title = element_text(size=22)) +
  ggtitle(name)
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_PrevelanceBoxplot.pdf'), plot = last_plot(), width = 8, height =5, dpi = 60, units = 'in')
...
qqplot3 <- function(qq, title = title, digits = 2,name = name, QQ= T,
                    ...
                    xlab = 'Prevelance (Raw dataset)',ylab = 'Prevelance (Compared)',xdensity = ''){
  colnames(qq)[2:4] = c('Dirichlet multinomial','Raw dataset','Semiparametric')
  cols = c(`Dirichlet multinomial` = '#3db7e4',Semiparametric = '#ff8849', `Raw dataset`= '#69be28')
  
  qq_23 = as.data.frame(stats::qqplot(qq[,'Raw dataset'], qq[,'Dirichlet multinomial'], plot.it=FALSE)) %>% dplyr::rename(`Dirichlet multinomial` = y, `Raw dataset` = x)
  qq_24 = as.data.frame(stats::qqplot(qq[,'Raw dataset'], qq[,'Semiparametric'], plot.it=FALSE)) %>% dplyr::rename(Semiparametric = y, `Raw dataset` = x)
  df = rbind(qq_23 %>% mutate(method ='Dirichlet multinomial') %>% dplyr::rename(value = `Dirichlet multinomial`),
  ...
  if(QQ){
    if(sqrtqq){
      qq234 = df %>% dplyr::mutate(`Raw dataset` = sqrt(`Raw dataset`), value = sqrt(value))  %>% ggplot() + 
        geom_abline(intercept = 0, color = 'grey10', cex = 1.5, alpha = 0.7) +
        geom_point(aes(x=`Raw dataset`, y=value, color = method)) +
  ...
  # qq23 = ggplot(qq_23) + 
  #   geom_abline(intercept = 0, color = 'grey10', cex = 1.5, alpha = 0.7) +
  #   geom_point(aes(x=`Raw dataset`, y=`Dirichlet multinomial`), color = brewer.pal(8,'Dark2')[3]) +
  ...
  #         legend.position = 'none')+
  #   labs(x='Raw dataset',y='Dirichlet multinomial')
  # qq24 = ggplot(qq_24) + 
  #   geom_abline(intercept = 0, color = 'grey10', cex = 1.5, alpha = 0.7) +
  #   geom_point(aes(x=`Raw dataset`, y= Semiparametric), color = 'forestgreen') +
  ...
  #         axis.title = element_text(color="black", size = 18),
  #         legend.position = 'none')+
  #   labs(x='Raw dataset',y='Semiparametric')
...
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_OTU prevalence.pdf'), plot = last_plot(), width = 8, height =4, dpi = 60, units = 'in')
qqplot3(qq1, title = 'Sparsity',xdensity = 'Sparsity', sqrt = F, xlab = 'Sparsity(Raw dataset)',ylab = 'Sparsity(Compared)', name = name)
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_Sparsity.pdf'), plot = last_plot(), width = 8, height =4, dpi = 60, units = 'in')
...
ggplot(melt(mn_abund %>% dplyr::rename(`Dirichlet multinomial`=DM,`Raw dataset`=raw,Semiparametric = dirchlet) %>% dplyr::filter(OTU %in% id1)),
...
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_OTU mean abundance violin.pdf'), plot = last_plot(), width = 10, height = 5, dpi = 60, units = 'in')
...
ggplot(mn_abund %>% filter(OTU %in% id))+
  ...
  xlab('Mean abundance (raw dataset)') +
  labs(fill = '') +theme_bw() +
  theme(axis.text = element_text(color="black", size = 26),
        ...
        legend.text = element_text(size=30),
        plot.title = element_text(size=22))
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_OTU mean abundance scatter.pdf'), plot = last_plot(), width = 10, height = 7, dpi = 60, units = 'in')

# id1: DM > raw variation; id: raw > DM variation
qqplot3(qq= mn_abund %>% filter(OTU %in% id),sqrt = T,sqrtqq = T,
        title = 'OTU mean abundance',digits=3, xlab = 'sqrt(Mean abundance (Raw dataset))', 
        ylab =  'sqrt(Abundance (Compared))', xdensity = 'sqrt(Mean abundance)', name = name)
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_OTU mean abundance_lowVarInRaw.pdf'), plot = last_plot(), width = 10, height = 5, dpi = 60, units = 'in')
qqplot3(qq= mn_abund %>% filter(OTU %in% id1),sqrt = T,sqrtqq = T,
        title = 'OTU mean abundance',digits=3, xlab = 'sqrt(Mean abundance (Raw dataset))', 
        ylab =  'sqrt(Abundance (Compared))', xdensity = 'sqrt(Mean abundance)', name = name)
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_OTU mean abundance_highVarInRaw.pdf'), plot = last_plot(), width = 10, height = 5, dpi = 60, units = 'in')

qqplot3(qq= mn_abund,sqrt = T,sqrtqq = T,
        title = 'OTU mean abundance',digits=3, xlab = 'sqrt(Mean abundance (Raw dataset))', 
        ylab =  'sqrt(Abundance (Compared))', xdensity = 'sqrt(Mean abundance)', name = name)
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_OTU mean abundance.pdf'), plot = last_plot(), width = 10, height = 5, dpi = 60, units = 'in')
...
# # DM > raw OTU in DM data
# pheatmap::pheatmap(rary2.r[id,], legend = TRUE, cluster_rows = F, 
...
#                    main = paste0('raw, var DM > raw'))
# 
# pheatmap::pheatmap(rary3.r[id,], legend = TRUE, cluster_rows = F, 
#                    cluster_cols = F,clustering_method = 'ward.D2',color = col.scheme,
#                    annotation_legend = F,show_colnames = F,show_rownames = F,fontsize_row =6,fontsize_col =6,
#                    main = paste0('DM, var DM > raw'))
# 
# pheatmap::pheatmap(rary2.r[id1,], legend = TRUE, cluster_rows = F, 
#                    cluster_cols = F,clustering_method = 'ward.D2',color = col.scheme,
#                    annotation_legend = F,show_colnames = F,show_rownames = F,fontsize_row =6,fontsize_col =6,
#                    main = paste0('raw, var raw > DM'))
# save_pheatmap_png(heat4, paste0('result/Simulation_comparison/',name,'_heat_SemiSimulation.png'))
# 
# pheatmap::pheatmap(rary3.r[id1,], legend = TRUE, cluster_rows = F, 
#                    cluster_cols = F,clustering_method = 'ward.D2',color = col.scheme,
#                    annotation_legend = F,show_colnames = F,show_rownames = F,fontsize_row =6,fontsize_col =6,
#                    main = paste0('DM, var raw > DM'))



ggplot(melt(var %>% dplyr::rename(`Dirichlet multinomial`=DM,`Raw dataset`=raw,Semiparametric = dirchlet) %>% filter(OTU %in% id1)),
       aes(x = variable, y = value, fill = variable)) + 
  geom_violin() +
  geom_boxplot(width = 0.1, fill = 'white', outlier.colour = NA) +
  ...
  scale_y_continuous(trans = sqrt_trans(),
                     breaks = trans_breaks("sqrt", function(x) x^2),
                     labels = trans_format("sqrt", math_format(.x^2))) +
  geom_hline(yintercept = median((var %>% dplyr::rename(`Dirichlet multinomial`=DM,`Raw dataset`=raw,Semiparametric = dirchlet) %>% filter(OTU %in% id1))[,'Raw dataset']), colour = 'red', linetype = 'dashed', size = 0.5) 
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_OTU abundance variation violin lowDM.pdf'), plot = last_plot(), width = 10, height = 6, dpi = 100, units = 'in')

ggplot(melt(var %>% dplyr::rename(`Dirichlet multinomial`=DM,`Raw dataset`=raw,Semiparametric = dirchlet) %>% filter(OTU %in% id)),
       aes(x = variable, y = value, fill = variable)) + 
  geom_violin() +
  geom_boxplot(width = 0.1, fill = 'white', outlier.colour = NA) +
  ylab('Variation of abundance') +
  ...
  scale_y_continuous(trans = sqrt_trans(),
                     breaks = trans_breaks("sqrt", function(x) x^2),
                     labels = trans_format("sqrt", math_format(.x^2))) +
  geom_hline(yintercept = median((var %>% dplyr::rename(`Dirichlet multinomial`=DM,`Raw dataset`=raw,Semiparametric = dirchlet) %>% filter(OTU %in% id))[,'Raw dataset']), colour = 'red', linetype = 'dashed', size = 0.5) 
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_OTU abundance variation violin highDM.pdf'), plot = last_plot(), width = 10, height = 6, dpi = 100, units = 'in')


qqplot3(var%>% filter(OTU %in% id1), title = 'OTU abundance variation',digits=3, sqrt = F,sqrtqq = F,
        xlab = 'sqrt(Variance of abundance (Raw dataset))', ylab =  'sqrt(Variance of abundance (Compared))', xdensity = 'sqrt(Variance of abundance)', name = name)
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_OTU abundance variation.pdf'), plot = last_plot(), width = 10, height = 5, dpi = 100, units = 'in')
...
clean.data <- function(data){
  data = data[rowSums(data!=0) > 0.1 * ncol(data), , drop=FALSE]
  data = data[rowMaxs(data) > 0.002, , drop=FALSE]
  return(data)
}
...
head(cor)
sink(paste0('result/CompareDMandSemiparametric/',name,'_correlation.txt'))
summary(cor)
...
qqplot3(cor, title = 'Taxa-Taxa(proportion 0.1% > 5% samples) correlation structure', sqrtqq = F, sqrt = F, QQ = T,
        xlab = 'Taxa-taxa correlation (Raw dataset)', 
        ylab =  'Taxa-taxa correlation (Compared)', 
        xdensity = 'Taxa-taxa correlation')
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_taxa-tax-correlation_filtered.pdf'), plot = last_plot(), width = 8, height =4, dpi = 60, units = 'in')
...
sink(paste0('result/CompareDMandSemiparametric/',name,'_Shannon.txt'))
cat('Raw median, max, min \n')
...
ggplot(melt(div0%>% dplyr::rename(`Dirichlet multinomial`=DM,`Raw dataset`=raw,Semiparametric = dirchlet)),
       aes(x = variable, y = value, fill = variable)) + 
  geom_violin() +
  geom_boxplot(width=0.1,outlier.size = 0, outlier.colour = 'grey30', fill = 'white') +
  ...
  theme(axis.text.x = element_blank(),
        ...
        plot.title = element_text(size=22)) +
  ggtitle(name)
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_alphaBoxplot.pdf'), plot = last_plot(), width = 8, height =5, dpi = 60, units = 'in')


qqplot3(div0, title = 'Shannon Diversity', xlab = 'Shannon diversity  (Raw dataset)', ylab =  'Shannon diversity (Compared)', xdensity = 'Shannon diversity', name = name)
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_alphadiversity1.pdf'), plot = last_plot(), width = 10, height = 5, dpi = 60, units = 'in')
...
sink(paste0('result/CompareDMandSemiparametric/',name,'_ANOSIM_DM.txt'))
ano
sink()
...
ano <- adonis2(ta.dist ~ grp, permutations = 999, method="bray")
sink(paste0('result/CompareDMandSemiparametric/',name,'_ANOSIM_Semiparametric.txt'))
ano
sink()



m2 = colnames(a2) %>% as.data.frame() %>% mutate(grp ='Raw dataset')
m3 = colnames(a3) %>% as.data.frame() %>% mutate(grp ='Dirichlet multinomial')
...
pcoa = ggplot(eng,aes(x = Axis.1, y = Axis.2,shape = grp, fill = grp)) +
  stat_ellipse(geom = "polygon", alpha = 0.2,type = "norm", color = '#999999')+ 
  geom_point(size = 2)+
  scale_color_manual(values = cols)+
...
pcoa
ggsave(paste0('result/CompareDMandSemiparametric/',name,'_PCOA.pdf'), plot = last_plot(), width = 8, height = 5, dpi = 60, units = 'in')
...
heat2 = pheatmap::pheatmap(r2,legend = TRUE, cluster_rows = F, 
                           ...
                           breaks = breaks,
                           annotation_legend = F,show_colnames = F,show_rownames = F,fontsize_row =6,fontsize_col =6,
                           main = paste0('Raw dataset'))
save_pheatmap_png(heat2, paste0('result/CompareDMandSemiparametric/',name,'_heat_raw.png'))

heat3 = pheatmap::pheatmap(r3,legend = TRUE, cluster_rows = F, 
                           cluster_cols = F,clustering_method = 'ward.D2', color = col.scheme,
                           breaks = breaks,
                           annotation_legend = F,show_colnames = F,show_rownames = F,fontsize_row =6,fontsize_col =6,
                           main = paste0('Dirichlet Multinomial'))

save_pheatmap_png(heat3, paste0('result/CompareDMandSemiparametric/',name,'_heat_DM.png'))
...
save_pheatmap_png(heat4, paste0('result/CompareDMandSemiparametric/',name,'_heat_SemiSimulation.png'))
