...
# dim(otu.tab)
load(paste0('Semi_',name,'.Rdata'))
load(paste0('DM_',name,'.Rdata'))
...
dim(rary2);dim(rary3);dim(rary4)
setdiff(colnames(rary2),colnames(rary4))
setdiff(colnames(rary2),colnames(rary3))
setdiff(rownames(rary2),rownames(rary3))
...
sink(paste0('result/Simulation_comparison/',name,'_Sparsity.txt'))
...
cbind(`Dirichlet multinomial` = zero3, `Raw dataset` = zero2, `Semiparametric` = zero4) %>% melt() %>% 
  ...
  ggtitle(name)
ggsave(paste0('result/Simulation_comparison/',name,'_Sparsity.pdf'), plot = last_plot(), width = 8, height =3.5, dpi = 60, units = 'in')
...
sink(paste0('result/Simulation_comparison/',name,'_Prevelance.txt'))
cat('Raw median, max, min \n')
...
ggplot(melt(qq),aes(x = variable, y = value, fill = variable)) + 
  ...
  theme(axis.text.x = element_blank(),
        ...
        plot.title = element_text(size=22)) +
  ggtitle(name)
ggsave(paste0('result/Simulation_comparison/',name,'_PrevelanceBoxplot.pdf'), plot = last_plot(), width = 8, height =5, dpi = 60, units = 'in')



cols = c(`Dirichlet multinomial` = '#3db7e4',Semiparametric = '#ff8849', `Raw dataset`= '#69be28')
qqplot3 <- function(qq, title = title, digits = 2,name = name, QQ= F,
  ...
  if(QQ){
    if(sqrtqq){
      qq234 = df %>% mutate(`Raw dataset` = sqrt(`Raw dataset`), value = sqrt(value))  %>% ggplot() + 
...
ggsave(paste0('result/Simulation_comparison/',name,'_OTU prevalence.pdf'), plot = last_plot(), width = 8, height =4, dpi = 60, units = 'in')
qqplot3(qq1, title = 'Sparsity',xdensity = 'Sparsity', sqrt = F, xlab = 'Sparsity(Raw dataset)',ylab = 'Sparsity(Compared)', name = name)
ggsave(paste0('result/Simulation_comparison/',name,'_Sparsity.pdf'), plot = last_plot(), width = 8, height =4, dpi = 60, units = 'in')
...
head(mn_abund)
ggplot(melt(mn_abund %>% dplyr::rename(`Dirichlet multinomial`=DM,`Raw dataset`=raw,Semiparametric = dirchlet) %>% filter(OTU %in% id1)),
...
ggsave(paste0('result/Simulation_comparison/',name,'_OTU mean abundance violin.pdf'), plot = last_plot(), width = 10, height = 5, dpi = 60, units = 'in')
...
ggplot(mn_abund %>% filter(OTU %in% id))+
  ...
  theme(axis.text = element_text(color="black", size = 26),
        ...
        legend.text = element_text(size=30),
        plot.title = element_text(size=22))
ggsave(paste0('result/Simulation_comparison/',name,'_OTU mean abundance scatter.pdf'), plot = last_plot(), width = 10, height = 7, dpi = 60, units = 'in')
...
ggsave(paste0('result/Simulation_comparison/',name,'_OTU mean abundance_lowVarInRaw.pdf'), plot = last_plot(), width = 10, height = 5, dpi = 60, units = 'in')
...
ggsave(paste0('result/Simulation_comparison/',name,'_OTU abundance variation violin lowDM.pdf'), plot = last_plot(), width = 10, height = 6, dpi = 100, units = 'in')
...
ggsave(paste0('result/Simulation_comparison/',name,'_OTU abundance variation violin highDM.pdf'), plot = last_plot(), width = 10, height = 6, dpi = 100, units = 'in')

ggsave(paste0('result/Simulation_comparison/',name,'_OTU abundance variation violin.pdf'), plot = last_plot(), width = 10, height = 6, dpi = 100, units = 'in')
...
ggsave(paste0('result/Simulation_comparison/',name,'_OTU abundance variation.pdf'), plot = last_plot(), width = 10, height = 5, dpi = 100, units = 'in')



########### correlation structure ########################
# taxa-taxa correlation
# tax2 = rary2.r[rowSums(rary2.r > 0.001) > (0.05 * ncol(rary2.r)), ]
tax2 = rary2.r
r.rary2.r = t(tax2)
is.matrix(r.rary2.r)
...
# tax3 = rary3.r[rowSums(rary3.r > 0.001) > (0.05 * ncol(rary3.r)), ];dim(tax3)
tax3 = rary3.r
r.rary3.r = t(tax3)
is.matrix(r.rary3.r)
...
# tax4 = rary4.r[rowSums(rary4.r > 0.001) > (0.05 * ncol(rary4.r)), ];dim(tax3)
tax4 = rary4.r
r.rary4.r = t(tax4)
is.matrix(r.rary4.r)
...
head(cor)
sink(paste0('result/Simulation_comparison/',name,'_correlation.txt'))
summary(cor)
...
qqplot3(cor, title = 'Taxa-Taxa(proportion 0.1% > 5% samples) correlation structure', 
        xlab = 'Taxa-taxa correlation (Raw dataset)', 
        ylab =  'Taxa-taxa correlation (Compared)', 
        xdensity = 'Taxa-taxa correlation')
ggsave(paste0('result/Simulation_comparison/',name,'_taxa-tax-correlation.pdf'), plot = last_plot(), width = 8, height =4, dpi = 60, units = 'in')
...
sink(paste0('result/Simulation_comparison/',name,'_Shannon.txt'))
cat('Raw median, max, min \n')
...
ggplot(melt(div0%>% dplyr::rename(`Dirichlet multinomial`=DM,`Raw dataset`=raw,Semiparametric = dirchlet)),
  ...
  theme(axis.text.x = element_blank(),
        ...
        plot.title = element_text(size=22)) +
  ggtitle(name)
ggsave(paste0('result/Simulation_comparison/',name,'_alphaBoxplot.pdf'), plot = last_plot(), width = 8, height =5, dpi = 60, units = 'in')
...
ggsave(paste0('result/Simulation_comparison/',name,'_alphadiversity1.pdf'), plot = last_plot(), width = 10, height = 5, dpi = 60, units = 'in')
...
sink(paste0('result/Simulation_comparison/',name,'_ANOSIM_DM.txt'))
ano
sink()
...
ano <- adonis2(ta.dist ~ grp, permutations = 999, method="bray")
sink(paste0('result/Simulation_comparison/',name,'_ANOSIM_Semiparametric.txt'))
ano
sink()
...
pcoa
ggsave(paste0('result/Simulation_comparison/',name,'_PCOA.pdf'), plot = last_plot(), width = 8, height = 5, dpi = 60, units = 'in')
...
TOPn = T
if(TOPn){
  top = names(sort(rowMeans(r2), decreasing = T))[1:885]
...
save_pheatmap_png(heat2, paste0('result/Simulation_comparison/',name,'_heat_raw.png'))
...
save_pheatmap_png(heat3, paste0('result/Simulation_comparison/',name,'_heat_DM.png'))
